...
function Quizzes() {
        ...
        if (fs.existsSync(path)) {
          ...
          while (i < content.length) {
            ...
            if (line == sQuestionEnd) {
              ...
              if (currentQuestion) {
                currentQuestion.addCorrectAnswer(
                  line.substr(sCorrectAnswer.length)
                );
              }
              ...
              try {
                if (currentQuestion)
                  currentQuestion.setTime(parseInt(line.substr(sTime.length)));
              } catch (e) {
              ...
              try {
                if (currentQuestion)
                  currentQuestion.setMarks(
                    parseInt(line.substr(sMarks.length))
                  );
              } catch (e) {
  ...
  this.addParticipant = function (participant) {
    ...
    if (quizId in quizzes) quizzes[quizId].addParticipant(participant);
  };
  ...
  this.isValidTeamname = function (quizId, teamname) {
    if (quizId in quizzes) return quizzes[quizId].isValidTeamname(teamname);
  };

  this.getQuizDetails = function (quizId) {
    if (quizId in quizzes) return quizzes[quizId].getDetails();
  };

  this.isValidQuizCode = function (quizId, quizCode) {
    if (quizId in quizzes) return quizzes[quizId].isValidQuizCode(quizCode);
  };
  ...
  this.sendUpdates = function (participant) {
    var quizId = participant.getQuizId();
    if (quizId in quizzes) quizzes[quizId].sendUpdates(participant);
  };

  this.getParticipant = function (quizId, participantId) {
    if (quizId in quizzes) return quizzes[quizId].getParticipant(participantId);
  };

  this.startQuiz = function (quizId) {
    if (quizId in quizzes) quizzes[quizId].startQuiz();
  };

  this.endQuiz = function (quizId) {
    if (quizId in quizzes) quizzes[quizId].endQuiz();
  };

  this.showTestQuestion = function (quizId) {
    if (quizId in quizzes) quizzes[quizId].showTestQuestion();
  };

  this.showNextQuestion = function (quizId) {
    if (quizId in quizzes) quizzes[quizId].nextQuestion();
  };
  ...
  this.cancelLastQuestion = function (quizId) {
    if (quizId in quizzes) quizzes[quizId].cancelLastQuestion();
  };

  this.getLeaderboard = function (quizId) {
    if (quizId in quizzes) return quizzes[quizId].getLeaderboard();
  };
...
function Participants() {
  ...
  this.getParticipant = function (participantId) {
    if (participantId in participants) return participants[participantId];
    else return false;
  };

  this.removeParticipant = function (uniqueId) {
    if (uniqueId in participants) delete participants[uniqueId];
  };
...
function Participant() {
  ...
  this.getUniqueId = function () {
    if (
      ...
      socket.handshake.session.unique_id
    ) {
  ...
  this.getQuizId = function () {
    if (
      ...
      socket.handshake &&
      socket.handshake.session &&
      socket.handshake.session.quiz_id
    ) {
...
function Leaderboard() {
  ...
  this.get = function () {
    ...
    for (var i = 0; i < official_participants.length; i++) {
      ...
      leaderboard["official"].push({
        team: entities.encode(p.getTeamname()),
        score: p.getScore(),
        rank: p.getRank(),
        isLastCorrect: p.isLastCorrect(),
      });
    }

    for (var i = 0; i < unofficial_participants.length; i++) {
      var p = unofficial_participants[i];
      leaderboard["unofficial"].push({
        team: entities.encode(p.getTeamname()),
        score: p.getScore(),
        rank: p.getRank(),
        isLastCorrect: p.isLastCorrect(),
  ...
  this.updateRank = function (participant) {
    ...
    if (participant.isOfficialParticipant)
      participantArray = official_participants;
    else if (participant.isUnofficialParticipant)
      participantArray = unofficial_participants;
...
function Quiz(pQuizId) {
  ...
  this.nextQuestion = function () {
    ...
    if (!(curState == states.STARTING || curState == states.SHOW_ANSWER))
      return;
  ...
  this.showTestQuestion = function () {
    ...
    testQuestion.setQuestion(
      "What is the name of the largest continent in the world?"
    );
    ...
    quizState.setHiddenParams({
      answerId: testQuestion.getAnswerId(),
      test: true,
  ...
  this.collectResponse = function (participant, response) {
    var curState = quizState.get();
    if (!(curState == states.SHOW_QUESTION || curState == states.TEST_QUESTION))
      return;
  ...
  this.showAnswer = function () {
    ...
    if (
      typeof hiddenParams != "undefined" &&
      typeof hiddenParams.test !== "undefined"
    )
      params.test = true;
  ...
  this.endQuiz = function () {
    var curState = quizState.get();
    if (!(curState == states.SHOW_QUESTION || curState == states.SHOW_ANSWER))
      return;
  ...
  this.setAdminCode = function (pAdminCode) {
    if (pAdminCode.length > 20)
      console.log(
        "# Invalid admin code. Must be less or equal to 20 characters"
      );
    else adminPassword = pAdminCode;
  };

  this.setQuizCode = function (pQuizCode) {
    if (pQuizCode.length > 20)
      console.log(
        "# Invalid quiz code. Must be less or equal to 20 characters"
      );
